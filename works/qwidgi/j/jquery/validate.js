...
jQuery.extend(jQuery.validator, {
	...
	defaults: {
		...
		onfocusin: function(element) {
			...
			// Добавлено для начала проверки поля при появлении фокуса
	...
	messages: {
		...
		maxlength: jQuery.format("Максимум {0} символов"),
		maxLength: jQuery.format("Максимум {0} символов"),
		minlength: jQuery.format("Минимум {0} символов"),
		minLength: jQuery.format("Минимум {0} символов"),
		rangelength: jQuery.format("Введите значение длиной от {0} до {1} символов"),
		rangeLength: jQuery.format("Введите значение длиной от {0} до {1} символов"),
		rangeValue: jQuery.format("Введите число от {0} до {1}"),
		range: jQuery.format("Введите число от {0} до {1}"),
		maxValue: jQuery.format("Введите число, меньшее или равное {0}"),
		max: jQuery.format("Введите число, меньшее или равное {0}"),
		minValue: jQuery.format("Введите число, большее или равное {0}"),
	...
	prototype: {
		...
		errorsFor: function(element) {
			var res = this.errors().filter("[@for='" + this.idOrName(element) + "']");
			if( !res.length )
				res = this.errors().filter("[@title='" + this.idOrName(element) + "']")
			return res;
		},
			...
			if ( previous.old !== value ) {
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						if (param && param.post) param.post( response );
					}
				});
				return "pending";
